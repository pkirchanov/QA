select * 
from salary s
join employee_salary es on es.es_id = s.s_id
join employees e on es.employee_id = e.e_id;

select *
from employees e
join employee_salary es on e.e_id = es.employee_id
join salary s on s.s_id = es.salary_id ; 
...
select employee_name, monthly_salary, role_name
 from employees e 
  join employee_salary es on es.employee_id = e.e_id 
  join salary s on es.salary_id = s.s_id 
  join roles_employee re on re.employee_id = e.e_id 
  join roles r on re.role_id = r.r_id 
   where role_name like '%Middle JavaScript%';
  
  select *
 from employee_salary es 
  join employees e on e.e_id = es.employee_id
  join salary s on s.s_id = es.salary_id
  join roles_employee re on re.employee_id = e.e_id
  join roles r on re.role_id = r.r_id
...
select *
 from employees e 
  left join employee_salary es on e.e_id = es.employee_id
  join salary s on s.s_id = es.salary_id
--  join roles_employee re on re.employee_id = e.e_id
--  join roles r on re.role_id = r.r_id
...
--select count(role_name) from roles r 
  ...
  select * from roles r 
full join roles_employee re on re.role_id = r.r_id
--join employees e on e.e_id = re.employee_id 
--where role_name like '%QA%';
  
  select * from salary s 
join employee_salary es on es.salary_id = s.s_id 
--full join employees e on e.e_id = es.employee_id; 
--where role_name like '%QA%';
...
join employees on employee_id = employees.s_id
join salary on salary_id = salary.id;

select  employee_name, monthly_salary from employee_salary 
join employees 
on employee_salary.employee_id = employees.s_id
join salary 
...
join employees E on employee_id = e.e_id
join salary S on salary_id = s.s_id;
...
left join employees e on e.e_id = es.employee_id
join salary s on es.salary_id = s.s_id 
where e.employee_name is null;

select * from salary s 
join employee_salary es on s.s_id = es.employee_id
where monthly_salary < 2000;
...
join employee_salary es on e.e_id = es.employee_id
right join salary s on es.salary_id = s.s_id 
where e.employee_name is null and s.monthly_salary is not null;

-- все зп без сотрудников

select e.employee_name, s.monthly_salary from employees e
full join employee_salary es on e.e_id = es.employee_id
full join salary s on es.salary_id = s.s_id 
where e.employee_name is null and s.monthly_salary is not null;
...
from employees e
left join employee_salary es on e.e_id = es.employee_id
left join salary s on es.salary_id = s.s_id
...
join roles on roles.s_id = role_id 
join employees on  employees.s_id = employee_id;
...
left join roles_employee re on re.role_id = r.r_id 
left join employees e on e.e_id = re.employee_id 
left join employee_salary es on es.employee_id = e.e_id
left join salary s on s.s_id = es.salary_id 
where role_name like '%Junior%';
...
join roles_employee re on re.role_id = r.r_id 
join employees e on e.e_id = re.employee_id 
join employee_salary es on es.employee_id = e.e_id
join salary s on s.s_id = es.salary_id 
...
--select max(s.monthly_salary) as "Max salary of QA engieers" from salary s
select * from salary s
join employee_salary es on es.salary_id = s.s_id
join roles_employee re on re.employee_id = es.employee_id 
join roles r on r.r_id = re.role_id 
--join employees e on e.e_id = re.employee_id 
--join employee_salary es on es.employee_id = e.e_id
--join salary s on s.s_id = es.salary_id 
--where role_name like '%QA engineer%';
...
from employees
join roles_employee on employees.s_id = roles_employee.employee_id
join roles on roles.s_id = roles_employee.role_id
where roles.role_name like '%developer';

select count(roles.role_name)
from roles_employee 
join roles on roles_employee.role_id = roles.s_id 
where roles.role_name like '%developer%';

-- 28. Вывести фонд (сумму) зарплаты разработчиков.

--select sum(s.monthly_salary) as "Sum salary of all Dev" 
select *
from roles r
join roles_employee re on re.role_id = r.r_id 
join employees e on e.e_id = re.employee_id 
...
join employees e on re.employee_id = e.e_id 
join roles r on re.role_id = r.r_id 
